...
def loadHapticSignals() -> list:
    ...
    with open('calibration_signals_patterns.json', 'r') as f:
        data = json.load(f)
...
file_name = f"Pat_{user_id}_{device_type}_{hand}.json"

# ------- GET HAPTIC SIGNAL LIST ---------
calibration_signals, haptic_signals = loadHapticSignals()
# print(calibration_signals)
# print(haptic_signals)
...
label_to_signal = {
    1: "D4_L",  # Pressure Level 1
    2: "D4_M",  # Pressure Level 2
    3: "D4_H"   # Pressure Level 3
    }
...
for signal in calibration_signals:
    ...
    print(f"Applying Pressure Level: {signal}")
...
plt.xlabel("Reported Pattern Level")
plt.ylabel("Actual Pattern Level")
plt.title("Confusion Matrix for Pattern Identification")

# Save in the same directory as the JSON file
conf_matrix_plot_filename = os.path.join(participant_dir, f"Pat_{user_id}_{device_type}_{display_id}_{hand}_conf_matrix.png")
...
response_time_plot_filename = os.path.join(participant_dir, f"Pat_{user_id}_{device_type}_{display_id}_{hand}_response_times.png")
...
file_name = f"{participant_dir}/Pat_{user_id}_{device_type}_{display_id}_{hand}.json"
